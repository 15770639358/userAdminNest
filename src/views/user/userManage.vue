...
<script>
import {getAllUserAndRoles, removeUserRole,getInfo, addUserRole, getCountUser, addUser, updatePassword, removeUserAndRole} from '../../api/user'
...
export default {
  ...
  methods: {
    async init() {
      const userInfo = await getAllUserAndRoles()
    ...
    async addUserRole(user) {
      ...
      this.showRoles = []
      let {data} = await getInfo({id:user.id})
      let rolesId = []
    ...
    async updatePass() {
      ...
      if(pass.trim().length <6){
        ...
        let {code} = await updatePassword({id: userId,password: md5(pass)})
        if(code === 0){
          this.$message({
            type: 'success',
            message: '修改成功'
